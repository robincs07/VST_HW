...
class MySportVGGNet(nn.Module):
    def __init__(self):
        super(MySportVGGNet, self).__init__()
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((7, 7))
        self.fc_block = nn.Sequential(
            nn.Linear(in_features = 7*7*256, out_features = 10)
            # nn.Dropout(0.1),
            # nn.Linear(in_features = 4096, out_features = 4096),
            # nn.ReLU(inplace=True),
            # nn.Dropout(0.1),
...
def main():
    ...
    print("Numbers of model's parameters: {}".format(num_params))
    ...
    plt.title("paras={}, acc_curve".format(num_params))
    plt.xlabel('epoch')
    plt.ylabel('acc')
